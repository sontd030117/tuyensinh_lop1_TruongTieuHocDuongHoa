    ...
    if rows:
        ...
        with col_del2:
            ...
            if st.button("❌ XÓA HỒ SƠ NÀY", use_container_width=True, type="primary"):
                if not df[df["student_name"] == student_to_delete].empty:
                    # ĐÃ FIX: Sửa lỗi bóc tách .iloc không bọc ngoặc vuông hàng dữ liệu
        ...
        if len(student_list) > 0:
            ...
            if not matched_data.empty:
                # ĐÃ FIX TRIỆT ĐỂ: Bóc tách dòng dữ liệu đầu tiên [0] rồi mới chuyển sang Dictionary để tránh lỗi AttributeError
                ...
                # Cổng xuất in ấn mở bung Tab mới chống rào cản bảo mật iframe của Streamlit Cloud
                ...
                <div style="text-align: center; margin-top: 40px;" class="no-print">
                    <button onclick="var pWin=window.open('','_blank'); pWin.document.write('<html><head><title>In phieu</title><style>@media print{{.no-print{{display:none !important;}}}}body{{margin:0;padding:40px;background:white;color:black;font-family:\\'Times New Roman\';}}</style></head><body>'+document.getElementById('print-area').innerHTML+'</body></html>'); pWin.document.close(); setTimeout(function(){'{{'}pWin.print();pWin.close();{'}}'},500);" style="width: 100%; max-width: 500px; padding: 14px; background-color: #0288d1; color: white; border: none; border-radius: 4px; font-size: 16px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.15);">🖨️ BẤM VÀO ĐÂY ĐỂ TIẾN HÀNH IN PHIẾU NÀY RA GIẤY A4</button>
                </div>
                </div></div>"""
