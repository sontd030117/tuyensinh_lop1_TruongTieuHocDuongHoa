    ...
    if rows:
        ...
        with col_del2:
            ...
            if st.button("❌ XÓA HỒ SƠ NÀY", use_container_width=True, type="primary"):
                if not df[df["student_name"] == student_to_delete].empty:
                    row_to_del = df[df["student_name"] == student_to_delete].iloc[0].to_dict()
        ...
        if len(student_list) > 0:
            ...
            if not matched_data.empty:
                # ĐÃ FIX DỨT ĐIỂM LỖI CRASH: Thêm bọc ngoặc vuông [0] trích xuất dòng đầu tiên chính xác
