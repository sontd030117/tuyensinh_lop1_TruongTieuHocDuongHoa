...
if password == "123456":  
    ...
    # Tạo trigger tải lại danh sách khi có thao tác xóa đơn
    ...
    if rows:
        ...
        # ----------------------------------------------------------------------
        # ĐÃ THÊM: KHỐI CHỨC NĂNG XÓA HỒ SƠ LỖI DO PHỤ HUYNH NHẬP SAI
        # ----------------------------------------------------------------------
        ...
        with col_del2:
            ...
            if st.button("❌ XÓA HỒ SƠ NÀY", use_container_width=True, type="primary"):
                # Lấy dòng bản ghi của học sinh được chọn để tìm ID và File ảnh tương ứng
                ...
                with st.spinner("⏳ Đang tiến hành xóa hồ sơ và giải phóng bộ nhớ đám mây..."):
                    try:
                        # 1. Gỡ file ảnh đính kèm khỏi Supabase Storage (nếu có) để dọn sạch bộ nhớ
                        ...
                        # 2. Xóa hàng dữ liệu ra khỏi bảng ho_so_tuyen_sinh
                        if record_id:
                        ...
                        st.toast("Hệ thống dữ liệu đã được cập nhật lại!", icon="🔄")
                        st.rerun() # Tải lại trang web để làm mới bảng số liệu ngay lập tức
        ...
        if len(student_list) > 0:
            ...
            if not matched_data.empty:
                ...
                st.write("")
                st.markdown("**📄 Bản mô phỏng xem trước khi in ra giấy A4 (Bám sát 100% mẫu đơn gốc trường):**")
