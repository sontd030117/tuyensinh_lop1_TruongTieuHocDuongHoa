...
if password == "123456":  
    ...
    if rows:
        ...
        # KHỐI CHỨC NĂNG XÓA HỒ SƠ LỖI DO PHỤ HUYNH NHẬP SAI
        ...
        with col_del2:
            ...
            if st.button("❌ XÓA HỒ SƠ NÀY", use_container_width=True, type="primary"):
                if not df[df["student_name"] == student_to_delete].empty:
                    # ĐÃ SỬA: Thay thế toán tử .iloc gây lỗi AttributeError bằng .iloc[0] để trích xuất an toàn dữ liệu hàng đầu tiên
        ...
        if len(student_list) > 0:
            ...
            if not matched_data.empty:
                # ĐÃ FIX DỨT ĐIỂM: Sử dụng .iloc[0].to_dict() để bóc tách hàng dữ liệu mượt mà, chống sập trang xem trước
                ...
                # GIẢI PHÁP ĐỘT PHÁ 100%: Mã JavaScript đóng gói HTML thành tệp Blob ảo trực tiếp trên lề trang, phá vỡ rào cản Pop-up blocker và ép máy in hoạt động lập tức
                html_footer = f"""<table style="width: 100%; margin-top: 10px; border-collapse: collapse; font-family: 'Times New Roman', Times, serif;"><tr><td style="width: 55%; vertical-align: top; text-align: left; font-size: 14.5px;"><b style="font-style: italic; display: block; margin-bottom: 6px;">*Hồ sơ kèm theo gồm:</b><div style="margin-bottom: 3px;">[✓] - Bản sao giấy khai sinh;</div><div style="margin-bottom: 3px;">[✓] - Số định danh;</div><div style="margin-bottom: 3px;">{f'[✓]' if img_url.startswith('http') else '[ ]'} - Bản photo thẻ Bảo hiểm y tế;</div><div style="margin-bottom: 3px;">[ ] - Giấy xác nhận diện chính sách (lớp 1);</div><div style="margin-bottom: 3px;">[ ] - Ảnh 2x3 (1 tấm);</div><div style="margin-bottom: 15px;">[ ] - Ảnh 3x4 (1 tấm);</div>{f'<div style="border: 1px dashed #999; padding: 4px; width: 142px; background-color: #fafafa;"><img src="{img_url}" width="132" alt="BHYT"></div>' if img_url.startswith('http') else ''}</td><td style="width: 45%; text-align: center; vertical-align: top;"><span style="font-size: 14.5px; font-style: italic; display: block; margin-bottom: 4px;">Dương Hòa, ngày...... tháng...... năm 2026</span><b style="font-size: 15px; letter-spacing: 0.5px; display: block;">CHỮ KÝ PHỤ HUYNH</b><span style="font-size: 12px; color: gray; font-style: italic; display: block; margin-top: 2px;">(Nét vẽ tay từ màn hình cảm ứng)</span><div style="margin: 8px 0;"><img src="{clean_sig}" width="180" style="mix-blend-mode: multiply;" alt="Chữ ký tay"></div><b style="text-transform: uppercase; font-size: 15px; letter-spacing: 0.5px; display: block; margin-top: 5px;">{actual_name}</b></td></tr></table>
                <div style="text-align: center; margin-top: 40px;" class="no-print">
                    <button onclick="var content=document.getElementById('print-area').innerHTML; var blob=new Blob(['<html><head><title>In phieu</title><style>body{{margin:0;padding:40px;background:white;color:black;font-family:\\'Times New Roman\\';}}</style></head><body onload=\\'window.print();setTimeout(function(){{window.close();}},500);\\'>'+content+'</body></html>'],{{type:'text/html'}}); var url=URL.createObjectURL(blob); var a=document.createElement('a'); a.href=url; a.target='_blank'; a.click();" style="width: 100%; max-width: 500px; padding: 14px; background-color: #0288d1; color: white; border: none; border-radius: 4px; font-size: 16px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.15);">🖨️ BẤM VÀO ĐÂY ĐỂ TIẾN HÀNH IN PHIẾU NÀY RA GIẤY A4</button>
                </div>
                </div></div>"""
