    ...
    if rows:
                ...
                if not df[df["student_name"] == student_to_delete].empty:
                    ...
                    with st.spinner("⏳ Đang tiến hành xóa hồ sơ..."):
                        try:
                            ...
                            if record_id:
                                supabase_client.table("ho_so_tuyen_sink").delete().eq("id", record_id).execute()
                            else:
        ...
        if len(student_list) > 0:
            ...
            if not matched_data.empty:
                ...
                # ĐÃ ĐỘT PHÁ SỬA LỖI IN: Mã JavaScript onclick ghi đè mở hẳn window.open() bung tab mới độc lập để máy tính cho phép bật cấu hình in 100%
