        ...
        if len(student_list) > 0:
            ...
            if not matched_data.empty:
                ...
                <div style="text-align: center; margin-top: 40px;" class="no-print">
                    <button onclick="var element=document.getElementById('print-area'); var opt={{margin:10, filename:'Phieu_tuyen_sinh_{selected_student}.pdf', image:{{type:'jpeg', quality:0.98}}, html2canvas:{{scale:2, useCORS:true}}, jsPDF:{{unit:'mm', format:'a4', orientation:'portrait'}} }}; html2pdf().set(opt).from(element).save();" style="width: 100%; max-width: 500px; padding: 14px; background-color: #4caf50; color: white; border: none; border-radius: 4px; font-size: 16px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,0.15);">📥 BẤM VÀO ĐÂY ĐỂ XUẤT FILE FILE PDF IN ĐƠN KHỔ A4</button>
                </div>
